...
class OmniGen2RotaryPosEmbed(nn.Module):
    ...
    def _get_freqs_cis(self, freqs_cis, ids: torch.Tensor) -> torch.Tensor:
        ...
        for i in range(len(self.axes_dim)):
            # if freqs_cis[i] is None:
            # print(f"freqs_cis[i] is None for i={i}")
            ...
            # print(f"ids.dtype: {ids.dtype}")
                # continue
            # print(f"freqs_cis[i].shape: {freqs_cis[i].shape}")
            # print(f"freqs_cis[i].device: {freqs_cis[i].device}")
            # print(f"freqs_cis[i].dtype: {freqs_cis[i].dtype}")
            # print(f"ids.shape: {ids.shape}")
            # print(f"ids.device: {ids.device}")
            # print(f"ids.dtype: {ids.dtype}")
