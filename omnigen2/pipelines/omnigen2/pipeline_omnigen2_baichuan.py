"""
OmniGen2 Diffusion Pipeline with Baichuan-Omni Model Support
...
class OmniGen2BaichuanPipeline(DiffusionPipeline, OmniGen2LoraLoaderMixin):
    """
    Pipeline for text-to-image generation using OmniGen2 with Baichuan-Omni model.

    This pipeline implements a text-to-image generation model that uses:
    - Baichuan-Omni for text encoding
    ...
    Args:
        ...
        mllm (AutoModelForCausalLM): The Baichuan-Omni text encoder model.
        processor: The processor for text processing.
    """
    ...
    def __init__(
        ...
        mllm: AutoModelForCausalLM,
        processor,
    ) -> None:
        """
        Initialize the OmniGen2 Baichuan pipeline.
        ...
        Args:
            ...
            mllm: The Baichuan-Omni text encoder model.
            processor: The processor for text processing.
        """
        super().__init__()
    ...
    def _get_baichuan_prompt_embeds(
        self,
        ...
        """
        Get prompt embeddings from the Baichuan-Omni text encoder.
        ...
        # For Baichuan-Omni, use the processor's tokenizer directly
        ...
        if untruncated_ids.shape[-1] >= text_input_ids.shape[-1] and not torch.equal(text_input_ids, untruncated_ids):
            ...
            logger.warning(
                f"The following part of your input was truncated because Baichuan-Omni can only handle sequences up to"
                f" {max_sequence_length} tokens: {removed_text}"
            )

        prompt_attention_mask = text_inputs.attention_mask.to(device)
        
        # For Baichuan-Omni, use the forward method which returns OmniMMCausalLMOutputWithPast
        model_output = self.mllm(
        ...
        if hasattr(model_output, 'hidden_states') and model_output.hidden_states is not None:
            prompt_embeds = model_output.hidden_states[-1]
        else:
            # Fallback: use the model's base forward pass to get hidden states
    ...
    def _apply_chat_template(self, prompt: str):
        """
        Apply chat template for Baichuan-Omni model.
        
        Args:
            prompt: The input prompt string
            
        Returns:
            str: The formatted prompt with chat template applied
        """
        # For Baichuan-Omni, check if the processor has apply_chat_template method
        if hasattr(self.processor, 'tokenizer') and hasattr(self.processor.tokenizer, 'apply_chat_template'):
            ...
            # Fallback: return prompt as is for Baichuan-Omni
            return prompt
        ...
        if hasattr(tokenizer, 'apply_chat_template'):
            prompt = tokenizer.apply_chat_template(prompt_messages, tokenize=False, add_generation_prompt=False)
        else:
            # Fallback: return original prompt if no chat template available
            prompt = prompt
    ...
    def encode_prompt(
        ...
        if prompt_embeds is None:
            prompt_embeds, prompt_attention_mask = self._get_baichuan_prompt_embeds(
                prompt=prompt,
                device=device,
        ...
        if do_classifier_free_guidance and negative_prompt_embeds is None:
            ...
            negative_prompt_embeds, negative_prompt_attention_mask = self._get_baichuan_prompt_embeds(
