...
class OmniGen2Pipeline(DiffusionPipeline, OmniGen2LoraLoaderMixin):
    ...
    def _get_qwen2_prompt_embeds(
        ...
        if untruncated_ids.shape[-1] >= text_input_ids.shape[-1] and not torch.equal(text_input_ids, untruncated_ids):
            ...
            logger.warning(
                "The following part of your input was truncated because Qwen2 can only handle sequences up to"
    ...
    def _apply_chat_template(self, prompt: str):
        ...
        prompt = self.processor.tokenizer.apply_chat_template(prompt_messages, tokenize=False, add_generation_prompt=False)
        return prompt
