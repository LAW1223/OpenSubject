...
def process_single_item(item, vie_score, max_retries=5):
    ...
    ground_truth_image = item['output_image']
    ...
    target_img_size = input_images[0].size
    # resize output image to target image size
    output_image = output_image.resize(target_img_size)
    ...
    if item['task_type'].find('manipulation') != -1:
        with_manipulation = True
    else:
        with_manipulation = False
    if item['task_type'].find('single') != -1:
        with_single = True
    else:
        with_single = False
    score_dict = vie_score.evaluate(input_images + [ground_truth_image] + [output_image], instruction, with_manipulation=with_manipulation)
...
if __name__ == "__main__":
    ...
    for task_type, sub_data in sub_datasets.items():
        result_list = []
        json_path = os.path.join(args.result_dir, "gpt4dot1", task_type, "score.jsonl")
        ...
        with ThreadPoolExecutor(max_workers=args.max_workers) as executor:
            ...
            for item in sub_data:
                key = item["key"]
                output_image_path = os.path.join(args.result_dir, "fullset", task_type, f"{key}.png")
    ...
    combined_json_path = os.path.join(args.result_dir, "gpt4dot1", "combined_score.jsonl")
